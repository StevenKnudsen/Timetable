package com.nobes.timetable.calendar.logic.reqhelp;
...
import static com.nobes.timetable.calendar.logic.reqhelp.ParseHelpService.countNums;
import static com.nobes.timetable.calendar.logic.reqhelp.ParseHelpService.pullDept;
            ...
            if ((preprocess.get(i).substring(0, 2).equals("or")) || (preprocess.get(i).substring(0, 2).equals("Or"))) {
                if (preprocess.get(i).length() > 6) {
                    ...
                    if (!dept.equals("-1")) {
                        ...
                        Matcher matcher = pattern.matcher(preprocess.get(i));
                        matcher.find();
                        int index = matcher.start();
                        String front = preprocess.get(i).substring(0, index);
                        String end = preprocess.get(i).substring(index);
                        preprocess.set(i, front + dept + " " + end);
                        preprocess.set(i - 1, preprocess.get(i - 1) + " " + preprocess.get(i));
                        preprocess.remove(i);
                    }
                }
    ...
    public ArrayList<String> preprocess(ArrayList<String> reqlist) {
        ...
        while (i < reqlist.size()) {
            ...
            if (reqlist.get(i).contains(":")) {
                int index = reqlist.get(i).indexOf(":");

                reqlist.set(i, reqlist.get(i).substring(index));
            }
